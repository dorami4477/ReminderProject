...
protocol NewAddTodoDelegate:AnyObject {
    func addTodo(data:NewTodo, folder:NewFolder)
}

final class NewAddViewController: BaseViewController {

    let mainView = AddView()
    weak var delegate:NewAddTodoDelegate?
    var priorty:Int = 2
    var deadLine:Int = GetDate.shared.todayInt
    let repository = TodoRepository()
    lazy var folder: NewFolder = repository.findFolder(date: deadLine)
    ...
    @objc func saveButtonTapped(){
        ...
        if !text.trimmingCharacters(in: .whitespaces).isEmpty{
            let data = NewTodo(title: text, content: mainView.contentTextView.text, registerDate: deadLine, memoTag:mainView.tagButtonView.contentLabel.text, priority: priorty)
            delegate?.addTodo(data: data, folder:folder)
    ...
    @objc func deadlineButtonTapped(){
        ...
        vc.deadLine = { value in
            ...
            //여기서 폴더 뷴류
            self.folder = self.repository.findFolder(date: value)
        }
...
extension NewAddViewController:PHPickerViewControllerDelegate{
